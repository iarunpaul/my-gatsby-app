# LinkedIn MCP Setup Script for Windows (PowerShell) - FIXED VERSION
# Run with: powershell -ExecutionPolicy Bypass -File setup_windows_fixed.ps1

Write-Host "Starting LinkedIn MCP Server Setup - Phase 1 (Windows)" -ForegroundColor Green
...
# Create LinkedIn MCP directory
Write-Host "Creating LinkedIn MCP directory..." -ForegroundColor Cyan
if (!(Test-Path "mcp-linkedin")) {
    New-Item -ItemType Directory -Path "mcp-linkedin"
} else {
    Write-Host "MCP LinkedIn directory already exists" -ForegroundColor Green
}
...
$packages = @("fastapi", "uvicorn", "anthropic", "python-dotenv", "requests", "pydantic")
...
$webServerContent | Out-File -FilePath "web_server.py" -Encoding UTF8
...
$startupContent | Out-File -FilePath "start_server.bat" -Encoding UTF8
...
$testContent | Out-File -FilePath "test_mcp.py" -Encoding UTF8
